...
"""
.Remove_Outlier(
                outlier_size=3).chk_feature_graph(
                select_type='outlier', feature='T8', original_compare=True, show=True)
corr(select_type='original', show=True)
Single_Compare_Graph(select_type='outlier', show=True, original_compare=True,
                                                     compare_csv_data_path=['eeg20-11-25-04.17.57/eeg.csv',
                                                                            'eeg20-11-25-04.38.20/eeg.csv',
                                                                            'eeg20-11-25-04.39.10/eeg.csv',
                                                                            'eeg20-11-25-04.40.36/eeg.csv',
                                                                            './eeg20-11-27-03.32.29/eeg.csv',
                                                                            'eeg20-11-27-03.48.04/eeg.csv',
                                                                            'eeg20-11-27-04.33.54/eeg.csv',
                                                                            './eeg20-11-27-12.52.15/eeg.csv',
                                                                            'eeg20-11-30-02.20.21/eeg.csv',
                                                                            'eeg20-11-30-02.34.28/eeg.csv',
                                                                            ])
...
.Remove_Outlier(
                outlier_size=3).
.create_label(outlier_size=2, remove_size=3, savpath=file_name, index=i)
.chk_feature_graph(
                select_type='outlier', feature='T8', original_compare=True, show=True,
                compare_csv_data_path=)

chk_feature_graph(
                select_type='outlier', feature='T8', original_compare=True, show=True,
                compare_csv_data_path=['./eeg20-11-27-03.32.29/eeg.csv', './eeg20-11-27-12.52.15/eeg.csv',
                                       './eeg20-11-27-03.48.04/eeg.csv'])

.Remove_Outlier(
                outlier_size=3).chk_feature_graph(
                select_type='outlier', feature='T8', original_compare=True, show=True,
                compare_csv_data_path=['./eeg20-11-27-03.32.29/eeg.csv','./eeg20-11-27-12.52.15/eeg.csv','./eeg20-11-27-03.48.04/eeg.csv'])



X
.corr(select_type='outlier', show=True)
choice(3, index=i)
Remove_Outlier(outlier_size=2.6).chk_feature_graph(
                select_type='outlier', feature='T8', original_compare=True, show=True,
                compare_csv_data_path=['./eeg20-11-27-03.32.29/eeg.csv','./eeg20-11-27-12.52.15/eeg.csv','./eeg20-11-27-03.48.04/eeg.csv','./eeg20-11-27-04.33.54/eeg.csv'])

['./eeg20-11-27-03.32.29/eeg.csv','./eeg20-11-27-12.52.15/eeg.csv','./eeg20-11-27-03.48.04/eeg.csv']

.chk_feature_graph(
                select_type='outlier', feature='T8', original_compare=True, show=True,
                compare_csv_data_path=['./eeg20-11-27-03.32.29/eeg.csv','./eeg20-11-27-12.52.15/eeg.csv','./eeg20-11-27-03.48.04/eeg.csv'])

f'./ADHD_dataSet/v{i}p.csv'
...
if __name__ == '__main__':
    ...
    feature = ['./All_Remove_Noise_ADHD_DataSet/' + i for i in os.listdir('All_Remove_Noise_ADHD_DataSet')]
    average = 0
    for i, l in enumerate(feature):
        try:
            a = preprocessing(l, features=['Fp1', 'Fp2', 'F3', 'F4', 'C3', 'C4', 'P3', 'P4', 'O1', 'O2', 'F7', 'F8', 'T7',
                                        'T8',
                                        'P7',
                                        'P8', 'Fz', 'Cz', 'Pz'])
            a.corr(show=False,ret=True)
    ...
    ax = plt.axes()
    ax.set_title('Mean Correlation by ADHD', fontsize=30)
    average=average / (i + 1)

    feature = ['./All_Remove_Noise_Control_DataSet/' + i for i in os.listdir('All_Remove_Noise_Control_DataSet')]
    average2 = 0
    for i, l in enumerate(feature):
        try:
            a = preprocessing(l, features=['Fp1', 'Fp2', 'F3', 'F4', 'C3', 'C4', 'P3', 'P4', 'O1', 'O2', 'F7', 'F8', 'T7',
    ...
    ax = plt.axes()
    ax.set_title('Mean Correlation by Control', fontsize=30)
    average2=average2/ (i + 1)

    corr_matrix = sns.heatmap(data=abs(average-average2), annot=True, fmt='.2f', cmap='Blues', ax=ax)
    plt.show()
